          ...
          >
            ...
            >
              <template slot="thead">
                ...
                <vs-th sort-key="client.name">Cliente</vs-th>
...
export default {
  ...
  methods: {
    search(search) {
      ...
      this.getReportInvoices(1, this.searchData);
    },
    changeRangeDates() {
      this.searchData.rangeDates = this.rangeDates.join(",");
      this.getReportInvoices(1, this.searchData, this.rangeDates);
    },
    goPage(page) {
      this.getReportInvoices(page, this.searchData);
    },
    async downloadExcel() {
    ...
    getReportInvoices(page = 1, search = {}) {
      this.loading = true;
      this.searchData = search;
      this.searchData.rangeDates = this.rangeDates.join(",");

      reportsService
        .getReportInvoices(page, search)
        .then(res => {
          this.loading = false;
          ...
          this.exonerated_amount = res.data.exonerated_amount;
        })
        .catch(error => {
          this.loading = false;
          /*this.$notify({
    ...
    async getSkipRegisters(page, search) {
      try {
        let datos = await reportsService.getReportInvoicesSkip(page, search);
